...
import { CalendarIcon, ChartIcon, CheckCircleIcon, ClockIcon } from "../../../shared/ui/icons";
import type { AulaResumo, DashboardAlunoResponse, FrequenciaTurmaAluno } from "../../../shared/types";
import {
  formatDate,
  formatDateTime,
  percentText,
...
const EChart = lazy(() => import("../../../shared/ui/EChart").then((module) => ({ default: module.EChart })));
...
const recentColumns: SimpleTableColumn<DashboardAlunoResponse["ultimas_presencas"][number]>[] = [
  {
    key: "disciplina",
    label: "Aula",
    render: (row) => (
      <div>
        <span className="cell-strong">{row.disciplina}</span>
        <div className="lesson-table-subtext">{formatDate(row.data)} · {formatTimeRange(row.inicio, row.fim)}</div>
      </div>
    ),
  },
  ...
  return (
    <Link className="lesson-list-item" to={lessonDetailPath(lesson.turma_id, lesson.aula_id)} key={lesson.aula_id}>
      <div>
        ...
        <span>{lesson.turma} · {lesson.sala}</span>
      </div>
...
export function StudentDashboardPage() {
  ...
  return (
    <>
      <PageMeta title="Aluno | AutoPonto" description="Dashboard acadêmica do aluno." />
      <div className="page-header">
        <div className="page-header-row">
          <div>
            <div className="page-pretitle">Aluno</div>
            <h1 className="page-title">Dashboard do aluno</h1>
            <p className="page-description">Acompanhe aulas, presença recente e frequência por turma.</p>
          </div>
        </div>
      </div>
      ...
      {data && (
        <>
          <div className="dashboard-kpi-grid academic-kpi-grid">
            <StatCard label="Frequência geral" value={percentText(data.resumo.percentual)} icon={<ChartIcon />} tone="green" subtext={`${data.resumo.presentes} presenças · ${data.resumo.ausentes} faltas`} />
            <StatCard label="Aulas fechadas" value={data.resumo.total_fechadas} icon={<CheckCircleIcon />} tone="blue" />
            <StatCard label="Pendentes" value={data.resumo.pendentes} icon={<ClockIcon />} tone="yellow" />
            <StatCard label="Aulas hoje" value={data.aulas_hoje.length} icon={<CalendarIcon />} tone="teal" />
          </div>
            ...
            <section className="card">
              <div className="card-header">
                <div>
                  ...
                  <div className="card-subtitle">Chamadas e horários do dia.</div>
                </div>
              </div>
              <div className="card-body lesson-list">
                {data.aulas_hoje.length > 0 ? data.aulas_hoje.map(lessonCard) : (
                  <EmptyState title="Nenhuma aula hoje" text="Quando houver aula hoje, ela aparecerá aqui." />
                )}
              </div>
            </section>
            ...
            <section className="card">
              <div className="card-header">
                <div>
                  ...
                  <div className="card-subtitle">Aulas planejadas ou abertas nos próximos dias.</div>
                </div>
              </div>
              <div className="card-body lesson-list">
                {data.proximas_aulas.length > 0 ? data.proximas_aulas.map(lessonCard) : (
                  <EmptyState title="Sem próximas aulas" text="A agenda futura da turma aparecerá nesta seção." />
                )}
              </div>
            </section>
          </div>
          ...
          <section className="card">
            <div className="card-header">
              <div>
                ...
                <div className="card-subtitle">Percentual calculado somente com aulas fechadas.</div>
              </div>
            </div>
            <div className="card-body">
              ...
              <SimpleTable
                ...
                emptyState={<EmptyState title="Sem frequência" text="Nenhuma aula fechada foi encontrada para suas turmas." />}
              />
            </div>
          </section>
          ...
          <section className="card">
            <div className="card-header">
              <div>
                ...
                <div className="card-subtitle">Aulas recentes com status resumido.</div>
              </div>
              <Link className="btn btn-outline btn-sm" to="/app/perfil">Ver detalhe</Link>
            </div>
            <div className="card-body">
              <SimpleTable
                ...
                emptyState={<EmptyState title="Sem presenças recentes" text="As presenças aparecerão após chamadas registradas." />}
              />
            </div>
          </section>
        </>
