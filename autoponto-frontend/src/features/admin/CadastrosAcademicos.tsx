...
export function CadastrosAcademicos(props: Props) {
  ...
  async function criarTurma(evento: FormEvent<HTMLFormElement>) {
    ...
    await apiFetch("/turmas/", {
      method: "POST",
      body: JSON.stringify({
        periodo_letivo: valorTexto(form, "periodo_letivo"),
        disciplina: valorTexto(form, "disciplina"),
        codigo: valorTexto(form, "codigo"),
        professores: professor ? [professor] : [],
        horarios: [{ sala: valorTexto(form, "sala"), horario_padrao: valorTexto(form, "horario_padrao") }],
        ativo: true,
      }),
    });
    evento.currentTarget.reset();
  ...
  return (
    <section className="panel-stack">
      ...
      <section className="panel form-section">
        ...
        <form className="form-grid compact" onSubmit={criarTurma}><SelectCampo name="periodo_letivo" label="Periodo" opcoes={periodoOpcoes} /><SelectCampo name="disciplina" label="Disciplina" opcoes={disciplinaOpcoes} /><Campo label="Codigo"><input name="codigo" placeholder="20261EECP0021" required /></Campo><SelectCampo name="professor" label="Professor" opcoes={professorOpcoes} required={false} /><SelectCampo name="sala" label="Sala" opcoes={salaOpcoes} /><SelectCampo name="horario_padrao" label="Horario padrao" opcoes={horarioOpcoes} /><Botao>Cadastrar turma</Botao></form>
      </section>
