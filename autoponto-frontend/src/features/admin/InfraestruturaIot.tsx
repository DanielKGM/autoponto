...
export function InfraestruturaIot({ nos, salas, dispositivos, status, diagnostico, onCriado }: Props) {
  async function criarNo(evento: FormEvent<HTMLFormElement>) {
    ...
    await apiFetch("/nos-borda/", { method: "POST", body: JSON.stringify({ codigo: valorTexto(form, "codigo"), nome: valorTexto(form, "nome"), interscity_uuid: valorTexto(form, "interscity_uuid"), ativo: true }) });
  ...
  async function criarDispositivo(evento: FormEvent<HTMLFormElement>) {
    evento.preventDefault();
    const form = new FormData(evento.currentTarget);
    await apiFetch("/dispositivos-esp32/", { method: "POST", body: JSON.stringify({ codigo: valorTexto(form, "codigo"), nome: valorTexto(form, "nome"), no: valorOpcional(form, "no"), sala: valorOpcional(form, "sala"), interscity_uuid: valorTexto(form, "interscity_uuid"), status: "offline", ativo: true }) });
    evento.currentTarget.reset();
  ...
  return (
    <section className="panel-stack">
      <section className="panel form-section">
        <h3>Nos e ESP32</h3>
        <form className="form-grid compact" onSubmit={criarNo}><Campo label="Codigo"><input name="codigo" placeholder="88A29E606012" required /></Campo><Campo label="Nome"><input name="nome" placeholder="raspberry-tcc" required /></Campo><Campo label="UUID IntersCity"><input name="interscity_uuid" placeholder="91723758-..." /></Campo><Botao>Cadastrar no</Botao></form>
        <form className="form-grid compact" onSubmit={criarDispositivo}><SelectCampo name="no" label="No" opcoes={opcoes(nos, (no) => `${no.codigo} - ${no.nome}`)} required={false} /><SelectCampo name="sala" label="Sala" opcoes={opcoes(salas, (sala) => `${sala.codigo} - ${sala.nome}`)} required={false} /><Campo label="Codigo"><input name="codigo" placeholder="9084CED6CDC0" required /></Campo><Campo label="Nome"><input name="nome" placeholder="esp32-tcc" required /></Campo><Campo label="UUID IntersCity"><input name="interscity_uuid" placeholder="8cf4ce45-..." /></Campo><Botao>Cadastrar ESP32</Botao></form>
      </section>
...
export function MapaDispositivos({ status }: { status: DispositivoStatus[] }) {
  ...
  return (
    <section className="panel map-panel">
      <div className="panel-header"><div><h3>Mapa operacional</h3><p>Visao demonstrativa dos dispositivos. A telemetria IntersCity vem do edge-node; a API mostra o ultimo snapshot local.</p></div><span className="badge info">Atualizacao manual</span></div>
      <div className="mapa-campus">
        {status.map((dispositivo, indice) => (
          <button key={dispositivo.id} type="button" onClick={() => setSelecionado(dispositivo)} className={`map-pin ${dispositivo.status_efetivo}`} style={{ left: `${18 + (indice % 4) * 20}%`, top: `${24 + Math.floor(indice / 4) * 22}%` }}>
            <strong>{dispositivo.nome}</strong>
            <span>{dispositivo.sala || "Sala nao definida"}</span>
            <small>{dispositivo.status_efetivo}</small>
          </button>
        ))}
        {status.length === 0 && <p className="map-empty">Nenhum dispositivo cadastrado.</p>}
      </div>
      {selecionado && <div className="map-detail"><strong>{selecionado.codigo}</strong><span>No: {selecionado.no || "-"}</span><span>Sala: {selecionado.sala || "-"}</span><span>UUID IntersCity: {selecionado.interscity_uuid || "-"}</span></div>}
    </section>
  );
}
