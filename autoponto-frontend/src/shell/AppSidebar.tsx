...
import { PopoverMenu, type PopoverMenuItem } from "../shared/ui/PopoverMenu";
import { AdminIcon, CalendarIcon, ChevronRightIcon, GraduationIcon, MapIcon, TeacherIcon, UserIcon } from "../shared/ui/icons";
...
export function AppSidebar() {
  ...
  function isCalendarContext(path: string) {
    return path === "/app/calendario" || path === "/app/turmas" || path.startsWith("/app/turmas/");
  }

  function isChildActive(parentArea: AreaApp, childPath: string) {
    if (childPath === "/app/calendario") {
      if (!isCalendarContext(location.pathname)) return false;
      if (me.usuario.papel === "ALUNO") return parentArea === "aluno";
      return parentArea === "professor";
    }
        ...
        <div className="nav-group">
          ...
          <ul className="nav-list">
            {itensNavegacao(me).map((item) => {
              ...
              const hasActiveChild = Boolean(item.children?.some((child) => isChildActive(item.area, child.path)));
                      ...
                      <div className="nav-sub-inner">
                        {item.children?.map((child) => (
                          <Link
                            ...
                            className={`nav-sublink ${isChildActive(item.area, child.path) ? "active" : ""}`}
                          >
                            {child.label}
                          </Link>
                        ))}
