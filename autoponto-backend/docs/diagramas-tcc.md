...
```mermaid
flowchart TB
    Backend["autoponto-backend/"]
    URLs["autoponto/api/urls.py\nrotas"]
    Views["autoponto/api/views\ncamada HTTP"]
    Serializers["autoponto/api/serializers\nvalidacao e JSON"]
    Services["autoponto/api/services\nregras de negocio"]
    Models["autoponto/api/models\ndominio e persistencia"]
    Tests["autoponto/api/tests\ncontratos e regressao"]
    Docs["docs\ndocumentacao do TCC"]
