...
def registrar_valor(
    ...
    try:
        ...
        logger.exception("falha ao registrar metrica tcc metrica=%s", metrica)


def _habilitado() -> bool:
    return bool(getattr(settings, "TCC_EVIDENCIAS_ENABLED", False))


def _diretorio() -> Path:
    return Path(getattr(settings, "TCC_EVIDENCIAS_DIR", "/data/logs/tcc"))
